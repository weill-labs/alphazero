...
# -----------------------------------------------------------------------------
# Tier 2: WDLP value head (W/D/L classification + auxiliary ply).
# -----------------------------------------------------------------------------


def _wdlp_cfg(**overrides) -> AlphaZeroNetConfig:
    return _transformer_cfg(value_head_style="wdlp", **overrides)


def test_wdlp_value_out_is_three_logits_and_has_ply_head() -> None:
    """wdlp value head outputs 3 W/D/L logits and adds a ply head; scalar has
    a single value logit and no ply_out — so checkpoints don't silently mix."""
    wdlp = create_model(_wdlp_cfg(), seed=0)
    scalar = create_model(_transformer_cfg(value_head_style="scalar"), seed=0)
    assert wdlp.value_out.kernel[...].shape[-1] == 3
    assert scalar.value_out.kernel[...].shape[-1] == 1
    assert wdlp.ply_out is not None
    assert scalar.ply_out is None


def test_wdlp_call_returns_scalar_value_in_range() -> None:
    """Inference contract: __call__ returns scalar value = P(win)-P(loss) in
    [-1, 1], so MCTS/cert are unchanged by the head swap."""
    cfg = _wdlp_cfg()
    model = create_model(cfg, seed=0)
    obs = jnp.zeros((4, *cfg.obs_shape), dtype=jnp.float32)
    policy, value = model(obs)
    assert policy.shape == (4, cfg.action_size)
    assert value.shape == (4,)
    assert jnp.all(jnp.abs(value) <= 1.0)


def test_wdlp_forward_train_shapes() -> None:
    """forward_train returns (policy, wdl_logits[B,3], ply_pred[B])."""
    cfg = _wdlp_cfg()
    model = create_model(cfg, seed=0)
    obs = jnp.zeros((5, *cfg.obs_shape), dtype=jnp.float32)
    policy, wdl_logits, ply_pred = model.forward_train(obs)
    assert policy.shape == (5, cfg.action_size)
    assert wdl_logits.shape == (5, 3)
    assert ply_pred.shape == (5,)


def test_wdlp_scalar_value_matches_softmax_expectation() -> None:
    """The __call__ scalar must equal softmax(wdl)·[-1,0,1] from forward_train."""
    cfg = _wdlp_cfg()
    model = create_model(cfg, seed=0)
    obs = jax.random.normal(jax.random.PRNGKey(1), (3, *cfg.obs_shape))
    _, value = model(obs)
    _, wdl_logits, _ = model.forward_train(obs)
    expected = jnp.sum(
        jax.nn.softmax(wdl_logits, axis=-1) * jnp.array([-1.0, 0.0, 1.0]), axis=-1
    )
    assert jnp.allclose(value, expected, atol=1e-6)


def test_resnet_rejects_wdlp() -> None:
    env = make_env()
    with pytest.raises(ValueError, match="requires arch='transformer'"):
        AlphaZeroNetConfig(
            obs_shape=initial_observation_shape(),
            action_size=env.num_actions,
            arch="resnet",
            value_head_style="wdlp",
        )


def test_wdlp_checkpoint_roundtrip(tmp_path) -> None:
    cfg = _wdlp_cfg(
        use_value_cls_token=True,
        policy_head_style="per_column",
        input_embed_style="conv3x3",
    ...
    ckpt = tmp_path / "wdlp.msgpack"
    save_checkpoint(model, ckpt)
    loaded = load_checkpoint(ckpt)
    assert loaded.config.value_head_style == "wdlp"
    after_p, after_v = loaded(obs)
    assert jnp.allclose(before_p, after_p, atol=1e-6)
    assert jnp.allclose(before_v, after_v, atol=1e-6)


def test_wdlp_end_to_end_train_is_finite() -> None:
    cfg = TrainingConfig(
        iterations=1,
        batch_size=4,
        num_simulations=2,
    ...
    result = run_training(cfg)
    assert result.net_config.value_head_style == "wdlp"
    assert jnp.isfinite(result.metrics[-1]["loss"])
